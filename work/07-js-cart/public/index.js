...
/* harmony export */   "getOneItemPrice": () => (/* binding */ getOneItemPrice),
...
function renderCartItems(cartItems) {
  ...
  cartItems.forEach(item => {
    ...
    itemTotalPrice.textContent = `Total price: $${(0,_helper__WEBPACK_IMPORTED_MODULE_0__.getOneItemPrice)(item).toFixed(2)}`;
  ...
  const totalPrice = (0,_helper__WEBPACK_IMPORTED_MODULE_0__.getTotalPrice)(cartItems);
  const totalQuantity = (0,_helper__WEBPACK_IMPORTED_MODULE_0__.getCartQuantity)(cartItems);
  const cartTotalPrice = document.createElement('h3');
  cartTotalPrice.setAttribute('id', 'cart-total-price');
  cartTotalPrice.textContent = `Total: $${totalPrice.toFixed(2)}, ${totalQuantity} cats`;
  cartBottom.appendChild(cartTotalPrice);
  cartItemQuantity.textContent = `(${(0,_helper__WEBPACK_IMPORTED_MODULE_0__.getCartQuantity)(cartItems)})`;
}
...
function handleItemsListClick(event) {
  ...
  if (viewCartBtnText.textContent === 'Hide Cart' && (0,_helper__WEBPACK_IMPORTED_MODULE_0__.getCartQuantity)(cartItems) !== 0) {
...
function handleChangeCartQuantityClick(event) {
  ...
  // Check if the item is already in the cart
  const existingItem = cartItems.find(cartItem => cartItem.id === parseInt(itemId));
