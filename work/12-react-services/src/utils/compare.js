...
export default function compare(word, guess) {
    // DO NOT MODIFY
    ...
    let wordCount = Array(26).fill(false),
        guessCount = Array(26).fill(false);
    ...
    for (let i = 0; i < word.length; i++) {
        if (!wordCount[word.charCodeAt(i) - 'a'.charCodeAt(0)]) {
            wordCount[word.charCodeAt(i) - 'a'.charCodeAt(0)] = true;
        }

        if (!guessCount[guess.charCodeAt(i) - 'a'.charCodeAt(0)]) {
